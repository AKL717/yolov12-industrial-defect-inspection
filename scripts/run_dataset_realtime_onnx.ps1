...
$DefaultOutput = Join-Path $ProjectRoot "results_dataset_realtime"
...
$DelayMs = if ($args.Count -ge 4) { $args[3] } else { "30" }
...
try {
    & $ExePath --backend onnx --model $ModelPath --input-size 640 --input $InputDir --output $OutputDir --show 1 --save 1 --evaluate 1 --delay-ms $DelayMs
} finally {
    Pop-Location
}
